...
use base qw(Pub::Ray::NET::b_sock Pub::Ray::NET::a_parser);
	...
	# checked or unchecked.
{
	...
	if ($checked)
	{
		...
		bless $service_port, 'Pub::Ray::NET::b_sock';
		$service_port->init();
		$service_port->start();
	}
	else
...
sub startImplementedService
{
    ...
    $service_port->start();
	Pub::Ray::NET::b_sock::incVersion();	# addition or deletion of implemented services causes h_server to send a new page
