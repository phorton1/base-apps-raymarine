...
{
	my ($class, $mon_defs) = @_;
	display($dbg_dp,0,"Pub::Ray::NET::e_DBNAV::newParser($mon_defs->{name})");
	...
	# a member 'text' field.
{
	...
	display($dbg_dp+2,0,"Pub::Ray::NET::e_DBNAV::parsePacket is_sniffer($is_sniffer) len($payload_len) num_fields($num_fields) ".
	...
	# Displays the previously parsed text if there is any
{
	...
	display($dbg_dp+1,0,sprintf("Pub::Ray::NET::e_DBNAV::parseMessage($len) mon(%04x)",$mon));
