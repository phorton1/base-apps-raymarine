# TRACK Protocol Discovery Notes

Raw discovery session material from early TRACK protocol analysis.
Contains probe definitions for non-working and exploratory commands,
annotated hex captures showing the discovery of the GET_DICT command,
MTA structure parsing cross-referenced with fshBlocks.pm field specs,
and track segment sentinel value identification (ffffffff ffffffff ffffffff ffff).

Source: extracted from NET/docs/reference/TRACK_probes.txt lines 106-362.
The current operational probe file is NET/probes/TRACK_probes.txt.
