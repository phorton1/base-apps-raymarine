...
description: Self-contained runbook for the nmOps test cycle (§1-§5). UUID table fully resolved 2026-05-08 from live /api/nmdb. Update UUID table only when baseline DB changes; all test steps reference [Names] only.
type: project
...
**PASTE_NEW of a route (confirmed 2026-05-08):** creates fresh route UUID; route_waypoints reference the SAME original WP UUIDs (per SS1.6, SS12.3) — no new WP records created. This is correct per the testplan (nmOps_testplan.md §2.10 line 250-251). The runbook's prior expected-behavior text "new member WPs (all fresh UUIDs)" was wrong.
...
- Record wall-clock start time before §1 reset (needed for last_testrun.md)
...
- **Never infer a result from a different test.** Each step is evaluated solely on whether
  ...
  the same guard via a different clipboard type. §5.4 (Cycle 10) is the canonical example:
  SS10.8 never fired; name-collision fired instead; recorded as PASSED_BUT citing §4.3 --
...
| [IsolatedWP3] | 994e0f7ef900baa4 | TOBOBE -- same parent branch; consumed by §2.3 delete |
| **WP referenced in a route** | | |
| [WPinRoute] | 314e56cc09005332 | Popa0 -- in Popa group (244e8e100800400a); pos=0 in Popa route |
| **Group with no route refs (used by §2.5 delete test)** | | |
| [GroupNoRoute] | a74e90d60300a434 | Bocas group -- Navigation/Waypoints (e54ede600200feee); 2 members (StarfishBeach + Fishfarm), none in route. Used for §2.5 (delete+WPs) ONLY. |
| **Group for §2.4 dissolve test** | | |
...
| **Group for §3.13 (paste new to E80) and §5.7 (ancestor-wins)** | | |
| [TestGroup] | 1a4eaf5a8c00e922 | Timiteo -- under oldE80/Groups ([UnsafeBranch]); 6 members (t01-t06), none in any route; survives all §2 tests; name "Timiteo" never conflicts with E80 state at §3.16 time (which has Popa from §3.12); §5.2 blocks [UnsafeBranch] delete so Timiteo is always safe; also used as group anchor in §2.16 |
| [TestGroupMember] | d44e40468d000d96 | t01 -- first member of [TestGroup]; used in §5.7 multi-select to trigger ancestor-wins |
...
| **Branch for §2.8 copy-branch test** | | |
...
| **Paste destination** | | |
| [DST] | 6f4e72ceae0264de | "AguaAndTobobe c 2011-03-06" -- under Michelle (034e6b8ccb01fffe); empty (0 WPs, 0 routes, 0 child collections); accumulates test output throughout §2 and §3 |
...
| [ChildBranch] | 984e7898480427f6 | MandalaLogs/Tracks -- direct child branch of [NestedBranch]; used as recursive paste target in §5.9 |
...
| [WP_A] | [dynamic §5.10] | First of two same-named WPs; find in /api/nmdb or create via UI |
| [WP_B] | [dynamic §5.10] | Second same-named WP (same name as [WP_A], different UUID) |
| [SameNameWP] | [dynamic §5.11] | DB WP with same name as [IsolatedWP1] ("BOCAS1"); create via UI if needed |

### E80 runtime UUIDs (derived during test run)

| [Name] | Derived at | How |
|--------|-----------|-----|
| [E80_WP] | §3.1 | /api/db waypoints after [IsolatedWP1] upload |
| [E80_GR] | §3.2 | /api/db groups after [GroupWithRouteMembers] upload |
| [E80_RT] | §3.3 | /api/db routes after [TestRoute] upload |
| [E80_TK1] | §4.0 | /api/db tracks after first teensyBoat track recorded |
| [E80_TK2] | §4.0 | /api/db tracks after second track recorded |
| [E80_RP1], [E80_RP2], [E80_RP3] | §3.19 | /api/db route_waypoints for fresh-UUID route from §3.17, sorted by position. [E80_RP1] MUST be Popa2 (454e11a80b002884) -- see §3.19 note. |
...
| StarfishBeach | 9d4e232a0500dd90 | member of Bocas group (= [GroupNoRoute]); deleted by §2.5 |
...
**§2.4/§2.5 ordering note:** Both tests need a group with no route refs. §2.4 (dissolve)
uses [GroupNoRoute_Dissolve] (Places/Part1, 4e4e405a08033af4) to preserve [GroupNoRoute]
(Bocas) for §2.5. Run §2.4 before §2.5.

---

### §2.1 Copy WP -> Paste New
...
### §2.4 Delete Group -- dissolve (members reparented)

Uses [GroupNoRoute_Dissolve] (Places/Part1), NOT Bocas, to preserve Bocas for §2.5 and §5.7.
```
...
Uses [GroupNoRoute] = Bocas (a74e90d60300a434), intact after §2.4.
```
...
### §2.6 Delete Group+WPS -- blocked (members in route)
...
Note: §2.17 says "in Michelle" -- stale after §2.9; UUID still valid, test still works.

---

### §2.10 Copy Route -> Paste New
...
Uses [IsolatedWP1] (ce4e43181f01b3ae) as source; [IsolatedWP2] (af4e23246d01bfa8) as position anchor
(now in [DST] after §2.2).

```
# PASTE_NEW_BEFORE (10304): fresh copy of IsolatedWP1 before IsolatedWP2
curl -s "http://localhost:9883/api/test?panel=database&select=[IsolatedWP1]&cmd=10010"
...
```
# PASTE_NEW_AFTER (10305): another fresh copy after IsolatedWP2
curl -s "http://localhost:9883/api/test?panel=database&select=[IsolatedWP1]&cmd=10010"
...
### §2.14 Paste Before/After -- route point (sequence reordering)

[TestRoute] is now in [DST] after §2.11 -- UUID unchanged.
Route point keys: `rp:[TestRoute]:[RPn]`

```
# Copy-splice: PASTE_NEW_BEFORE (10304) -- insert duplicate of RP1 between RP2 and RP3
...
```
# Cut-splice: PASTE_BEFORE (10302) -- reorder RP3 to before RP2
...
[TestRoute] = Popa route (f34efdd6070022e8, now in [DST] after §2.11). [IsolatedWP1] (ce4e43181f01b3ae) is still in its original location (oldE80/Tracks), untouched.

```
# PASTE_NEW_BEFORE (10304): fresh WP before the route object in [DST] ordering
curl -s "http://localhost:9883/api/test?panel=database&select=[IsolatedWP1]&cmd=10010"
...
```
# PASTE_NEW_AFTER (10305): fresh WP after the route object in [DST] ordering
curl -s "http://localhost:9883/api/test?panel=database&select=[IsolatedWP1]&cmd=10010"
...
[SomeBranch] = MichellToKuna 2011-07 (784e76f880029e1e), in Michelle (034e6b8ccb01fffe). After §2.9 its contents were moved to [DST]; the branch shell still exists. [IsolatedWP1] as clipboard source.
...
**Route clipboard:** [IsolatedWP2] (af4e23246d01bfa8) is in [DST] after §2.2; it is the positional anchor.

```
# PASTE_NEW_BEFORE (10304): fresh route copy before [IsolatedWP2] in [DST] ordering
curl -s "http://localhost:9883/api/test?panel=database&select=[TestRoute]&cmd=10010"
curl -s "http://localhost:9883/api/test?panel=database&select=[IsolatedWP2]&right_click=[IsolatedWP2]&cmd=10304"
```
...
**Group clipboard:** [TestGroup] = Timiteo (1a4eaf5a8c00e922); [IsolatedWP2] as anchor.

```
# PASTE_NEW_BEFORE (10304): fresh group copy before [IsolatedWP2] in [DST] ordering
...
E80 must be connected and empty after §1 reset. Run these after §2 completes cleanly.

**State note entering §3:** [TestRoute] (f34efdd6070022e8) is now in [DST] after §2.11 -- select by UUID still works. [GroupWithRouteMembers] = Popa group (244e8e100800400a) is intact (§2.6 was blocked).

**§3.1-§3.3 dual role:** §3.1, §3.2, and §3.3 each populate E80 with data that later §3 steps
...
Member WPs must already be on E80 -- §3.2 put them there; SS10.10 pre-flight verifies this.
```
curl -s "http://localhost:9883/api/test?panel=database&select=[TestRoute]&cmd=10010"
...
### §3.4 Copy E80 WP -> Sync to DB (sync-classified)
...
Prerequisite: §3.3 has run; [E80_GR] (Popa, 244e8e100800400a) and [E80_RT] (Popa route,
...
(§3.6b verified this). Deleting routes first clears the dependency.
```
...
### §3.8 Delete via E80 Groups header (all groups) -- SS8.2
...
§3.8 deleted all groups -- re-upload Popa group first:
```
...
```
Wait for ProgressDialog FINISHED. Then delete via specific group node:
```
curl.exe -s "http://localhost:9883/api/test?panel=e80&select=[E80_GR]&right_click=[E80_GR]&cmd=10421"
```
Wait for ProgressDialog FINISHED. Expected: [E80_GR] and all member WPs (11 Popa WPs) absent.

---

### §3.10 Delete via E80 My Waypoints (all ungrouped WPs) -- SS8.2

If [E80_WP] was deleted in §3.6, re-upload [IsolatedWP1] (ce4e43181f01b3ae) first.
```
curl -s "http://localhost:9883/api/test?panel=e80&select=my_waypoints&right_click=my_waypoints&cmd=10421"
```
Wait for ProgressDialog FINISHED. Expected: all ungrouped WPs deleted; named groups unaffected.

---

### §3.11 Delete via E80 Tracks header -- SS8.2

Requires tracks on E80. If teensyBoat not available, defer to §4 or mark NOT_RUN.
```
curl -s "http://localhost:9883/api/test?panel=e80&select=header%3Atracks&right_click=header%3Atracks&cmd=10440"
```
Wait for ProgressDialog FINISHED. Expected: all E80 tracks erased.

---

### §3.12 Copy E80 Group -> Sync to DB

If §3.9 deleted [E80_GR], re-upload Popa group (244e8e100800400a) first:
```
curl.exe -s "http://localhost:9883/api/test?panel=database&select=244e8e100800400a&cmd=10010"
curl.exe -s "http://localhost:9883/api/test?panel=e80&select=header%3Agroups&right_click=header%3Agroups&cmd=10300"
```
Wait for ProgressDialog FINISHED. Then sync from E80 to DB (clipboard_class='sync'; cmd=10600):
```
...
### §3.13 Copy E80 Route -> Sync to DB

Re-upload [TestRoute] to E80 if absent (deleted by §3.7):
```
curl -s "http://localhost:9883/api/test?panel=database&select=[TestRoute]&cmd=10010"
curl -s "http://localhost:9883/api/test?panel=e80&select=header%3Aroutes&right_click=header%3Aroutes&cmd=10300"
```
...
### §3.15b Copy E80 fresh-UUID WP -> Paste to DB (paste-classified: absent)

The §3.15 PASTE_NEW created a WP on E80 with a fresh UUID (byte 1 = 0x82) NOT in DB.
...
```
# COPY the fresh-UUID WP from E80 (find its UUID in /api/db waypoints after §3.15)
...
### §3.15c Mixed-classified E80 clipboard: status bar + PASTE_NEW

After §3.15, E80 has [E80_WP] (UUID in DB) and [E80_FRESH_WP] (UUID not in DB).
Select both → clipboard_class='mixed'.

```
curl -s "http://localhost:9883/api/test?panel=e80&select=[E80_WP],[E80_FRESH_WP]&cmd=10010"
```
...
Survives all §2 tests. Name "Timiteo" never conflicts with E80 state at §3.16 time (E80 has Popa from §3.12/§3.13).
```
curl -s "http://localhost:9883/api/test?panel=database&select=[TestGroup]&cmd=10010"
curl -s "http://localhost:9883/api/test?panel=e80&select=header%3Agroups&right_click=header%3Agroups&cmd=10301"
```
...
Note: leaves Timiteo group (and members) on E80; §5.6 clears all groups before its route-dependency test.

---

### §3.17 Paste New Route to E80 (fresh route UUID, WP refs preserved)

If a same-named route is on E80, delete all routes first:
```
curl -s "http://localhost:9883/api/test?panel=e80&select=header%3Aroutes&right_click=header%3Aroutes&cmd=10430"
```
Wait for ProgressDialog FINISHED. Then:
...
```
Wait for ProgressDialog FINISHED.
Expected: new route on E80 with fresh UUID (byte 1=0x82); member WPs (from §3.12) referenced by UUID; no new WP records created.

---

### §3.18 Multi-select WPs -> Paste to E80 (homogeneous flat set)

**WARNING:** After §3.15, BOCAS2 (name "BOCAS2") is on E80 with a FRESH UUID (3a4eb590d40403ae),
...
on E80, OR delete BOCAS2 from E80 first before running §3.18. Do not reuse [IsolatedWP1]+[IsolatedWP2].
...
### §3.19 Route point Paste Before/After on E80

Requires a route on E80 with 3+ waypoints. After §3.17, the fresh-UUID route from PASTE_NEW
serves as the source. Derive its route UUID from /api/db routes at this point.
Route point keys: `rp:[E80_RT]:[E80_RP_UUID]`

**KNOWN ISSUE -- Popa0 is duplicated:** The §3.17 fresh-UUID route inherits the §2.14
...
- [E80_RP1] = Popa2 (454e11a80b002884) -- confirmed unique in the post-§2.14 route
...
## §4 Track Tests (requires teensyBoat on port 9881)

Skip entirely if teensyBoat not available; mark all §4 steps NOT_RUN.
Load boat_driving_guide.md before starting.

### §4.0 Create test tracks on E80

Drive to create at least two short tracks. Note E80-assigned UUIDs as [E80_TK1], [E80_TK2].
Track creation: AP=0 -> H=NNN -> S=50 -> start -> drive -> stop -> name -> save.
Non-fatal expected: GET_CUR2 ERROR after EVENT(0); "TRACK OUT OF BAND" after save.
Verify save: "got track(uuid) = 'name'" in log.

**UUID behavior:** E80->DB paste does NOT preserve E80 UUID. navMate assigns fresh UUID.
Search by name in /api/nmdb, not by UUID.
Color conversion: E80 index -> aabbggrr: 0=ff0000ff, 1=ff00ffff, 2=ff00ff00, 3=ffff0000, 4=ffff00ff, 5=ff000000.

---

### §4.1 Copy E80 Track -> Paste to DB (download, track remains on E80)

```
curl -s "http://localhost:9883/api/test?panel=e80&select=[E80_TK1]&cmd=10010"
curl -s "http://localhost:9883/api/test?panel=database&select=[DST]&right_click=[DST]&cmd=10300"
```
Wait for ProgressDialog FINISHED. Expected: track in DB with fresh UUID; still on E80.
...
curl -s "http://localhost:9883/api/test?panel=database&select=[DST]&right_click=[DST]&cmd=10300"
```
Wait for ProgressDialog FINISHED. Expected: track in DB; TRACK_CMD_ERASE sent; absent from /api/db.

---

### §4.3 Guard -- Paste Track to E80 blocked (SS10.8)

[TestTrack] is now in [DST] after §2.12, UUID unchanged.
```
...
(SS10.3 guard for DB-to-DB track copy is tested in §5.5, not here.)
...
### §5.1 DEL_WAYPOINT blocked -- WP referenced in route
...
### §5.2 DEL_BRANCH blocked -- member WP in external route
...
**Pre-condition:** [IsolatedWP1] (BOCAS1, ce4e43181f01b3ae) and [IsolatedWP2] (BOCAS2) are
on E80 after §3.18. If either is still present, the E80-wide name collision guard fires before
SS10.8 -- the test then does NOT verify what it is supposed to.

Check /api/db for BOCAS1 and BOCAS2; delete each if present before proceeding:
```
# Delete BOCAS1 from E80 if present (UUID ce4e43181f01b3ae)
curl -s "http://localhost:9883/api/test?panel=e80&select=ce4e43181f01b3ae&right_click=ce4e43181f01b3ae&cmd=10410"
# Wait for ProgressDialog FINISHED; check /api/db to confirm gone
```
(BOCAS2 may have a fresh UUID from §3.15 PASTE_NEW -- find it via /api/db name match and delete similarly.)
...
[TestTrack] is in [DST] after §2.12, UUID unchanged.
```
curl -s "http://localhost:9883/api/test?panel=database&select=[TestTrack]&cmd=10010"
curl -s "http://localhost:9883/api/test?panel=database&select=[DST]&right_click=[DST]&cmd=10300"
```
...
### §5.6 Route dependency check -- paste route before member WPs exist on E80 (SS10.10)

§3.13 left Timiteo group (and possibly other content from §3.x) on E80. Clear it all first:
```
curl -s "http://localhost:9883/api/test?panel=e80&select=header%3Aroutes&right_click=header%3Aroutes&cmd=10430"
# Wait for ProgressDialog FINISHED
curl -s "http://localhost:9883/api/test?panel=e80&select=header%3Agroups&right_click=header%3Agroups&cmd=10421"
# Wait for ProgressDialog FINISHED
curl -s "http://localhost:9883/api/test?panel=e80&select=my_waypoints&right_click=my_waypoints&cmd=10421"
# Wait for ProgressDialog FINISHED (no-op if no ungrouped WPs remain)
```
Verify /api/db returns empty E80.

Then paste [TestRoute] (Popa route, 11 WPs -- none are on E80 after the clear above):
```
curl -s "http://localhost:9883/api/test?panel=database&select=[TestRoute]&cmd=10010"
curl -s "http://localhost:9883/api/test?panel=e80&select=header%3Aroutes&right_click=header%3Aroutes&cmd=10300"
```
Expected: WARNING listing missing WP UUIDs (11 Popa WPs); E80 routes unchanged.
Note: E80 is now empty; §5.7 pastes Timiteo fresh from this clean state.
...
Prerequisite: E80 is empty (§5.6 cleared it; the Timiteo group from §3.13 is gone).
```
...
### §5.8 Ancestor-wins -- abort path -- PREREQUISITE: outcome=reject

Mark NOT_RUN until `$suppress_outcome` is implemented (see todo.md).

---

### §5.9 Recursive paste guard -- paste Branch into its own descendant
...
```
# Upload IsolatedWP1 (BOCAS1) to E80
curl -s "http://localhost:9883/api/test?panel=database&select=[IsolatedWP1]&cmd=10010"
curl -s "http://localhost:9883/api/test?panel=e80&select=header%3Agroups&right_click=header%3Agroups&cmd=10300"
# Wait for ProgressDialog FINISHED

# Attempt paste of a different WP also named "BOCAS1"
...
### §5.12 UUID conflict -- clean create path

Covered by §3.15. Verify §3.15 log shows no conflict-resolution dialog. No separate command.

---

### §5.13 UUID conflict -- conflict dialog path -- PREREQUISITE: outcome=reject + versioning wired
...
With a clipboard loaded (COPY any WP), fire PASTE at a DB waypoint node (object node, not a collection):
```
curl -s "http://localhost:9883/api/test?panel=database&select=[IsolatedWP1]&cmd=10010"
...
Fire PASTE_NEW at the same object node:
```
curl -s "http://localhost:9883/api/test?panel=database&select=[IsolatedWP1]&right_click=[IsolatedWP1]&cmd=10301"
```
Expected: IMPLEMENTATION ERROR in log; DB unchanged.

Also fire PASTE at the [TestRoute] object node and at the [TestTrack] object node:
```
...
```
Expected: IMPLEMENTATION ERROR for both; DB unchanged.

---

### §5.15 Menu shape -- E80 WP node: all paste items absent

E80 must have at least one WP; upload [IsolatedWP1] if E80 is empty:
```
curl -s "http://localhost:9883/api/test?panel=database&select=[IsolatedWP1]&cmd=10010"
curl -s "http://localhost:9883/api/test?panel=e80&select=header%3Agroups&right_click=header%3Agroups&cmd=10300"
```
Wait for ProgressDialog FINISHED. Load clipboard then fire PASTE at the E80 WP UUID directly:
```
curl -s "http://localhost:9883/api/test?panel=database&select=[IsolatedWP1]&cmd=10010"
curl -s "http://localhost:9883/api/test?panel=e80&select=[E80_WP]&right_click=[E80_WP]&cmd=10300"
```
Expected: `ERROR - Cannot paste: destination is a descendant of an item in the clipboard`.
E80 unchanged.
Note: [E80_WP] has the same UUID as [IsolatedWP1] in the clipboard. The _doPaste Step 3
UUID-match guard fires first and correctly rejects. The _pasteE80 individual-WP-node guard
is never reached; do NOT look for "individual waypoint node destination" here.

Fire PASTE_NEW at the same E80 WP node:
```
curl -s "http://localhost:9883/api/test?panel=e80&select=[E80_WP]&right_click=[E80_WP]&cmd=10301"
```
Expected: same `ERROR - Cannot paste: destination is a descendant of an item in the clipboard`.
E80 unchanged.
...
"Mixed" here = clipboard has both route_point AND waypoint items (SS6.4 accepted case).
Note: [RP1] is Popa0. Popa0 may appear more than once in [TestRoute] after §2 operations --
check route point count before this test and use that as the baseline.

Load mixed clipboard (1 route_point + 1 waypoint):
```
curl -s "http://localhost:9883/api/test?panel=database&select=rp:[TestRoute]:[RP1],[IsolatedWP1]&cmd=10010"
```
Fire PASTE_BEFORE (10302) at route_point anchor [RP2]:
```
curl -s "http://localhost:9883/api/test?panel=database&select=rp:[TestRoute]:[RP2]&right_click=rp:[TestRoute]:[RP2]&cmd=10302"
```
Expected: succeeds. No IMPLEMENTATION ERROR. Both [RP1] (route_point) and [IsolatedWP1]
(waypoint) are inserted as route_waypoints entries before [RP2] using their WP UUIDs.
Route count increases by clipboard item count (N items in clipboard = +N in route).

Re-load clipboard and fire PASTE_NEW_BEFORE (10304) at same anchor:
```
curl -s "http://localhost:9883/api/test?panel=database&select=rp:[TestRoute]:[RP1],[IsolatedWP1]&cmd=10010"
...
route_waypoints entries before [RP2] using their existing WP UUIDs. No new waypoint
...
- §3.16 must run before §5.7: §3.16 leaves Timiteo on E80; §5.6 clears E80; §5.7 tests a
  fresh Timiteo paste from a clean E80. If §3.16 is NOT_RUN, §5.7 can still run but E80 must
  be verified empty first.
- §2.3 consumes [IsolatedWP3] (TOBOBE); don't use 994e0f7ef900baa4 after §2.3.
- §2.7 deletes [SafeBranch] (Before Sumwood Channel); §2.17 uses [SomeBranch] as the branch
  anchor (not [SafeBranch]); §2.9 empties [SomeBranch] but it persists as a node.
- §2.11 moves [TestRoute] to [DST]; UUID (f34efdd6070022e8) still valid for §2.14, §2.15,
  §2.18, and §3.x.
- §2.12 moves [TestTrack] to [DST]; UUID (1a4eed924904ebbe) still valid for §4.3 and §5.5.
- §2.16 uses [TestGroup] (Timiteo, 1a4eaf5a8c00e922) as group anchor; §3.16 uses it for
  PASTE_NEW to E80; §5.7 uses it for ancestor-wins. All three uses are independent (none
  destructive to [TestGroup] itself).
- §3.15b and §3.15c require §3.15 to run first (they need a fresh-UUID WP on E80). The
  fresh-UUID WP is [E80_FRESH_WP] -- derive its UUID from /api/db after §3.15 runs (it is
...
**Section numbers in last_testrun.md MUST match the runbook exactly.** The runbook is the
canonical source of section numbers. Do not use numbers from memory, prior testruns, or the
testplan prose. Copy the §X.Y label from the runbook heading for each step you execute.
